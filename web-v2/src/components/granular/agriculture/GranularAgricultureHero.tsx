...
export default function GranularAgricultureHero() {
  ...
  return (
    <section id="agricultura" className="relative w-full min-h-[70svh] md:min-h-[90svh] bg-[#050505] overflow-hidden flex flex-col justify-end pb-12 border-b border-white/10 pt-16">
      
      {/* BACKGROUND TEXTURE */}
      <div className="absolute inset-0 z-0 bg-[#050505]">
        <motion.div className="w-full h-full relative" {...imgAnim}>
           {/* Transition from water blue to agriculture orange/yellow */}
           <div className="absolute inset-0 bg-gradient-to-br from-[#050505] via-[var(--granular-dim-agriculture)]/5 to-[#050505] mix-blend-screen z-10" />
           <Image 
            src="/portfolio-media/audit/block-15/page-25-audit.png" // Fallback to audit representation
            alt="Fondo conceptual de agricultura"
            fill
      ...
      <div className="relative z-20 w-full px-6 md:px-12 flex flex-col gap-8 md:gap-16">
        ...
        <motion.div className="flex items-center gap-4" {...textAnim}>
          ...
          <span className="w-1.5 h-1.5 rounded-full bg-[var(--granular-dim-agriculture)]" />
          <span className="text-mono text-[10px] text-[var(--granular-dim-agriculture)] uppercase tracking-widest">
            {granularAgriculture.title}
          </span>
        </motion.div>

        <motion.div className="flex flex-col gap-4 max-w-4xl" {...textAnim}>
          <h2 className="text-display-lg leading-[0.9] text-white">
            <span className="block text-white/80">EL SISTEMA PRODUCTIVO</span>
            <span className="block text-[var(--granular-dim-agriculture)]">Y SUS DEPENDENCIAS TERRITORIALES</span>
          </h2>
        </motion.div>
        ...
        <motion.div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pt-8 border-t border-white/10" {...textAnim}>
          <div className="flex gap-8">
            <div>
              ...
              <div className="text-mono text-[12px] text-white/80">{granularAgriculture.sourcePages.join('–')}</div>
            </div>
            <div>
              <div className="text-mono text-[9px] text-white/40 uppercase tracking-widest mb-1">Escalas</div>
              <div className="text-mono text-[12px] text-white/80 uppercase">Regional · Municipal · Localidad</div>
            </div>
          </div>
          
          <div className="flex items-center gap-6">
             <a href="#sistema-agroproductivo" className="text-mono text-[10px] text-white hover:text-[var(--granular-dim-agriculture)] uppercase tracking-widest transition-colors flex items-center gap-2">
                Explorar →
             </a>
          </div>
        </motion.div>
