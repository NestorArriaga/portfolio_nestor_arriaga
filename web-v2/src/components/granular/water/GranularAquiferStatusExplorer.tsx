"use client";

import { useState } from "react";
import Image from "next/image";
import { granularWater } from "@/content/cases/granular/granular-water";
import ImageViewer from "@/components/case-study/ImageViewer";
import { useSafeMode } from "@/components/home/motion/SafeModeContext";

export default function GranularAquiferStatusExplorer() {
  const { aquiferSection, aquiferCategories } = granularWater;
  const [activeCategory, setActiveCategory] = useState<string | null>(null);
  const [isViewerOpen, setIsViewerOpen] = useState(false);
  const safeMode = useSafeMode();

  return (
    <section id="acuiferos" className="w-full py-24 md:py-32 flex flex-col gap-12 border-b border-white/10">
      
      <div className="flex flex-col gap-4 max-w-3xl px-6 md:px-0">
        <h3 className="text-display-md text-white">
          <span className="block text-white/80">{aquiferSection.title}</span>
          <span className="block text-[var(--granular-dim-water)]">{aquiferSection.subtitle}</span>
        </h3>
        <p className="text-mono text-[10px] text-white/40 uppercase tracking-widest">
          PÁGINA {aquiferSection.sourcePage}
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        
        {/* SELECTOR DE ACUÍFEROS */}
        <div className="lg:col-span-5 flex flex-col gap-6 px-6 md:px-0 order-2 lg:order-1">
          
          <div className="mb-4 p-4 bg-amber-500/5 border border-amber-500/20 rounded-[var(--radius-panel)] flex flex-col gap-2">
            <span className="text-mono text-[9px] text-amber-500 uppercase tracking-widest font-bold">
              NOTA SOBRE LA CLASIFICACIÓN
            </span>
            <span className="text-xs text-white/70">
              Los rangos se reproducen conforme a la página fuente. La definición textual de Crítico y Sobreexplotado presenta una superposición que no puede resolverse con la información disponible.
            </span>
          </div>

          <div className="text-mono text-[10px] text-white/50 uppercase tracking-widest mb-2">
            Estado de presión (extracción / recarga)
          </div>

          <div className="flex flex-col gap-4" role="tablist">
            {aquiferCategories.map((cat) => {
              const isActive = activeCategory === cat.id;
              
              if (safeMode) {
                return (
                  <div key={cat.id} className="p-4 border border-white/10 bg-[#111] rounded-[var(--radius-panel)]">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <span className="w-3 h-3 rounded-full" style={{ backgroundColor: cat.color }} />
                        <span className="text-mono text-[11px] text-white uppercase tracking-widest">{cat.name}</span>
                      </div>
                      <span className="text-mono text-[10px] text-white/40">{cat.range}</span>
                    </div>
                    <p className="text-sm text-white/70">{cat.desc}</p>
                  </div>
                );
              }

              return (
                <button
                  key={cat.id}
                  onClick={() => setActiveCategory(isActive ? null : cat.id)}
                  role="tab"
                  aria-current={isActive ? "true" : undefined}
                  className={`w-full text-left p-4 rounded-[var(--radius-panel)] transition-all duration-300 border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--granular-dim-water)] ${isActive ? 'bg-white/10 border-white/20' : 'bg-transparent border-white/5 hover:bg-white/5'}`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <span 
                        className="w-3 h-3 rounded-full transition-all duration-300" 
                        style={{ 
                          backgroundColor: cat.color,
                          boxShadow: isActive ? `0 0 12px ${cat.color}60` : 'none'
                        }} 
                      />
                      <span className={`text-mono text-[11px] uppercase tracking-widest transition-colors ${isActive ? 'text-white' : 'text-white/60'}`}>
                        {cat.name}
                      </span>
                    </div>
                    <span className={`text-mono text-[10px] transition-colors ${isActive ? 'text-white/60' : 'text-white/30'}`}>
                      {cat.range}
                    </span>
                  </div>
                  
                  <div className={`overflow-hidden transition-all duration-500 ${isActive ? 'max-h-64 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                    <p className="text-sm text-white/80">{cat.desc}</p>
                    {cat.id === 'critico' && (
                      <p className="mt-4 text-xs text-[var(--granular-dim-water)]/90 border-l-2 border-[var(--granular-dim-water)]/50 pl-3">
                        El texto del portafolio señala déficits que superan el 150% respecto a la recarga en sectores del oriente de Durango. La página no muestra junto a esta afirmación la serie, el año, el volumen ni el procedimiento de cálculo.
                      </p>
                    )}
                  </div>
                </button>
              );
            })}
          </div>

        </div>

        {/* MAPA Y VISOR */}
        <div className="lg:col-span-7 flex flex-col gap-4 order-1 lg:order-2">
          <button 
            onClick={() => setIsViewerOpen(true)}
            className="w-full relative aspect-[4/3] md:aspect-square lg:aspect-[4/3] rounded-[var(--radius-panel)] overflow-hidden border border-white/10 group bg-[#0A0A0A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--granular-dim-water)] focus-visible:ring-offset-4 focus-visible:ring-offset-[#050505]"
            aria-label="Abrir mapa de acuíferos en visor completo"
          >
            <Image
              src="/portfolio-media/audit/block-17/page-24-aquifers-audit.png" // Fallback to audit image
              alt={aquiferSection.caption}
              fill
              className="object-contain transition-transform duration-700 group-hover:scale-[1.02]"
              unoptimized
            />
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
               <div className="opacity-0 group-hover:opacity-100 bg-black/80 text-white text-mono text-[10px] uppercase tracking-widest px-4 py-2 rounded-full border border-white/20 transition-opacity duration-300 transform translate-y-4 group-hover:translate-y-0">
                 Ampliar mapa
               </div>
            </div>
          </button>
          
          <div className="flex flex-col gap-2 px-2">
            <p className="text-body text-sm text-white/70">
              {aquiferSection.caption}
            </p>
            <p className="text-mono text-[9px] text-white/40">
              {aquiferSection.sourceNote}
            </p>
          </div>
        </div>

      </div>
      
      {/* GEOGRAFÍA DE LA ESCASEZ */}
      <div className="mt-8 px-6 md:px-0">
         <div className="max-w-3xl p-6 md:p-8 bg-[#111] border border-white/5 rounded-[var(--radius-panel)]">
            <h4 className="text-mono text-[10px] text-white/50 uppercase tracking-widest mb-4">
              UNA GEOGRAFÍA DESIGUAL DE PRESIÓN HÍDRICA
            </h4>
            <p className="text-sm md:text-base text-white/80 leading-relaxed">
              {aquiferSection.scarcityGeoNote}
            </p>
         </div>
      </div>

      <ImageViewer 
        isOpen={isViewerOpen}
        onClose={() => setIsViewerOpen(false)}
        highResSrc="/portfolio-media/audit/block-17/page-24-aquifers-audit.png"
        altText={aquiferSection.caption}
      />
    </section>
  );
}
