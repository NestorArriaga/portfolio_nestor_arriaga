...
export default function GranularGovernanceLimitations() {
  const { limitations } = granularGovernance;

  return (
    <section id="alcance-gobernanza" className="w-full py-24 md:py-32 flex flex-col gap-8 border-b border-white/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-x-12 md:gap-y-6 mt-4 px-6 md:px-0 max-w-5xl">
        {limitations.points.map((point, idx) => (
          <div key={idx} className="flex gap-4 items-start">
             <span className="text-mono text-[10px] text-[var(--granular-dim-governance)] mt-1">0{idx+1}</span>
