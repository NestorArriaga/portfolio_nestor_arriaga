...
export default function GranularSystemHero() {
  ...
  return (
    <section id="sistema-territorial" className="relative w-full min-h-[70svh] md:min-h-[90svh] bg-[#050505] overflow-hidden flex flex-col justify-end pb-12 border-b border-white/10 pt-16">
      
      {/* BACKGROUND TEXTURE */}
      <div className="absolute inset-0 z-0 bg-[#050505]">
        <motion.div className="w-full h-full relative" {...imgAnim}>
           <Image 
            src="/portfolio-media/audit/block-22/page-40-causal-loop-audit.png" // Fallback to audit causal loop
            alt="Fondo introductorio de sistema territorial"
            fill
      ...
      <div className="relative z-20 w-full px-6 md:px-12 flex flex-col gap-8 md:gap-16">
        ...
        <motion.div className="flex items-center gap-4" {...textAnim}>
          ...
          <span className="text-mono text-[10px] text-white uppercase tracking-widest">
            {granularSystem.title}
          </span>
        </motion.div>

        <motion.div className="flex flex-col gap-4 max-w-4xl" {...textAnim}>
          <h2 className="text-display-lg leading-[0.9] text-white">
            <span className="block text-white/80">UNA REPRESENTACIÓN</span>
            <span className="block text-white">MULTIESCALAR Y MULTIDIMENSIONAL</span>
          </h2>
        </motion.div>
        ...
        <motion.div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pt-8 border-t border-white/10" {...textAnim}>
          <div className="flex gap-8">
            <div>
              ...
              <div className="text-mono text-[12px] text-white/80">{granularSystem.sourcePages.join('–')}</div>
            </div>
            <div>
          ...
          <div className="flex items-center gap-6">
             <a href="#causal-loop" className="text-mono text-[10px] text-white hover:text-white/70 uppercase tracking-widest transition-colors flex items-center gap-2">
                Explorar →
             </a>
          </div>
        </motion.div>
