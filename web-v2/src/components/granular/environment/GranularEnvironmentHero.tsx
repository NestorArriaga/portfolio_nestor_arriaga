...
export default function GranularEnvironmentHero() {
  ...
  return (
    <section id="ambiente" className="relative w-full min-h-[70svh] md:min-h-[90svh] bg-[#050505] overflow-hidden flex flex-col justify-end pb-12 border-b border-white/10 pt-16">
      
      {/* BACKGROUND TEXTURE */}
      <div className="absolute inset-0 z-0 bg-[#050505]">
        <motion.div className="w-full h-full relative" {...imgAnim}>
           {/* Transition using magenta */}
           <div className="absolute inset-0 bg-gradient-to-br from-[#050505] via-[var(--granular-dim-environment)]/20 to-[#050505] mix-blend-screen z-10" />
           <Image 
            src="/portfolio-media/audit/block-20/page-34-environment-audit.png" // Fallback to audit
            alt="Fondo conceptual de ambiente"
            fill
      ...
      <div className="relative z-20 w-full px-6 md:px-12 flex flex-col gap-8 md:gap-16">
        ...
        <motion.div className="flex items-center gap-4" {...textAnim}>
          ...
          <span className="w-1.5 h-1.5 rounded-full bg-[var(--granular-dim-environment)]" />
          <span className="text-mono text-[10px] text-[var(--granular-dim-environment)] uppercase tracking-widest">
            {granularEnvironment.title}
          </span>
        </motion.div>

        <motion.div className="flex flex-col gap-4 max-w-4xl" {...textAnim}>
          <h2 className="text-display-lg leading-[0.9] text-white">
            <span className="block text-white/80">SOPORTE BIOFÍSICO</span>
            <span className="block text-[var(--granular-dim-environment)]">Y LÍMITES ECOLÓGICOS</span>
          </h2>
        </motion.div>
        ...
        <motion.div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pt-8 border-t border-white/10" {...textAnim}>
          <div className="flex gap-8">
            <div>
              ...
              <div className="text-mono text-[12px] text-white/80">{granularEnvironment.sourcePages.join('–')}</div>
            </div>
            <div>
          ...
          <div className="flex items-center gap-6">
             <a href="#limites-ecologicos" className="text-mono text-[10px] text-white hover:text-[var(--granular-dim-environment)] uppercase tracking-widest transition-colors flex items-center gap-2">
                Explorar →
             </a>
          </div>
        </motion.div>
