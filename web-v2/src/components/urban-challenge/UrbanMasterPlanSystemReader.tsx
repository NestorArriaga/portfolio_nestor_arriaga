...
export default function UrbanMasterPlanSystemReader() {
  const [activeIndex, setActiveIndex] = useState(0);
  const [viewerOpen, setViewerOpen] = useState(false);
  const { systemReader } = urbanMasterPlan;

  const currentItem = systemReader[activeIndex];
  const currentAsset = urbanAssets.find(a => a.id === currentItem.assetId);

  return (
    <div className="w-full flex flex-col md:flex-row gap-12 bg-[#0a0a0a] border border-white/10 rounded-sm p-6 md:p-12">
      ...
      <div className="w-full md:w-1/3 flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2" role="tablist">
          {systemReader.map((item, index) => (
            ...
            >
              <span className="text-mono text-[9px] mr-3">{String(index + 1).padStart(2, '0')}</span>
              {item.title}
            </button>
          ))}
        </div>
        ...
        <div className="mt-4 flex flex-col gap-4">
          ...
          {currentItem.limitation && (
            <p className="text-[11px] font-light text-[var(--urban-accent)] bg-[var(--urban-accent)]/10 p-3 rounded-sm border border-[var(--urban-accent)]/20 leading-relaxed">
              {currentItem.limitation}
            </p>
          )}
        </div>
      ...
      <div className="w-full md:w-2/3 flex flex-col gap-4">
        <div 
          className="w-full aspect-square md:aspect-[4/3] relative bg-[#111] rounded border border-white/5 flex items-center justify-center p-8 cursor-zoom-in"
