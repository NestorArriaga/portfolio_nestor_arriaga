...
export default function UrbanMasterPlanHero() {
  const safeMode = useSafeMode();
  const prefersReducedMotion = useReducedMotion();
  const shouldAnimate = !safeMode && !prefersReducedMotion;
  
  const heroAsset = urbanAssets.find(a => a.id === urbanMasterPlan.hero.assetId);
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col gap-16 px-6">
        ...
        <div className="flex flex-col gap-4 text-center md:text-left">
          <span className="text-mono text-[10px] text-white/40 tracking-widest uppercase">
            CAPÍTULO {urbanMasterPlan.chapter.number}
          </span>
          <h2 className="text-4xl md:text-6xl font-light text-white leading-tight">
            {urbanMasterPlan.chapter.title}
          </h2>
          <h3 className="text-xl md:text-2xl font-light text-[var(--urban-accent)]">
            {urbanMasterPlan.chapter.subtitle}
          </h3>
        </div>

        {/* Hero Visual */}
        {heroAsset && (
          <div className="w-full aspect-[4/3] md:aspect-[16/9] relative overflow-hidden bg-[#111] rounded border border-white/10 group flex items-center justify-center p-8">
            <Image
              src={heroAsset.previewPath}
              alt={urbanMasterPlan.hero.alt}
              fill
              className={`object-contain p-4 md:p-12 transition-transform duration-[2000ms] ${shouldAnimate ? "group-hover:scale-[1.02]" : ""}`}
              priority
            />
            ...
            <div className="absolute bottom-4 right-4 flex items-center gap-3">
              <span className="text-mono text-[9px] text-white/50 tracking-widest uppercase bg-black/50 px-2 py-1 rounded backdrop-blur-sm">
                PÁGINA {urbanMasterPlan.chapter.page}
              </span>
              <span className="text-mono text-[9px] text-white/50 tracking-widest uppercase bg-black/50 px-2 py-1 rounded backdrop-blur-sm border border-white/10">
