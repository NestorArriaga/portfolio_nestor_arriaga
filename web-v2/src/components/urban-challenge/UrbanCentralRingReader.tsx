...
export default function UrbanCentralRingReader() {
  const [activeIndex, setActiveIndex] = useState(0);
  const activeComponent = urbanCentralRing.system.components[activeIndex];

  const perspectiveAsset = urbanAssets.find(a => a.id === urbanCentralRing.hero.perspectiveAssetId);
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row border border-white/10 rounded-sm overflow-hidden bg-black/20">
      ...
      <div className="w-full lg:w-1/3 flex flex-col border-b lg:border-b-0 lg:border-r border-white/10">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-xs text-white/70 font-light leading-relaxed">
            {urbanCentralRing.system.description}
          </p>
        </div>
        
        <div className="flex flex-col flex-1 p-4 gap-1">
          {urbanCentralRing.system.components.map((comp, idx) => (
            <button
              key={comp.id}
      ...
      <div className="w-full lg:w-2/3 flex flex-col">
        <div className="relative w-full aspect-[4/3] bg-[#030303] flex items-center justify-center p-8 border-b border-white/10">
          {perspectiveAsset && (
